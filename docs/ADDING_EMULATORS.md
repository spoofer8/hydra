...
7. **Forgetting the preload bridge.** IPC endpoints are exposed to the renderer in THREE places — `src/main/events/emulators/<name>.ts` (registers the handler), `src/preload/index.ts` (forwards via `ipcRenderer.invoke`), and `src/renderer/src/declaration.d.ts` (TypeScript signature). Miss the preload one and typecheck stays green because `declaration.d.ts` lies to TypeScript, but at runtime `window.electron.<name>` is undefined and any component that calls it throws `TypeError: window.electron.X is not a function`. Any new event added under `src/main/events/` needs a matching line in `src/preload/index.ts`.
8. **ROM scan finds 0 games even though the extensions are correct.** `src/main/services/emulators/scan-rom-folder.ts` has a `dedupGames()` function that branches by `binary.system` — the `else` fallback applies PS1's `PRIMARY_EXTS` set (`.cue`, `.iso`, `.chd`, `.pbp`, …). If your emulator's ROMs don't have any of those extensions (Switch is `.nsp`/`.xci`, 3DS is `.3ds`/`.cia`, etc.), the collect phase picks the files up but dedupGames throws them all away. Add a system branch with an `apply<System>Rules` helper — for single-file formats it's a one-liner that filters by extension and wraps each match in `{ primary, sidecars: [] }`. Also check `classifyForSystem` — if your extensions are in `SNIFFABLE_EXTS`, the disc-image sniffer will run and may flag them wrong-platform.
9. **ROM scan finds files, but LaunchBox matches 0 of them.** `src/main/services/emulators/extract-disc-sku.ts:extractDiscSku()` branches by system to pull a SKU (LaunchBox lookup key) from each file. Without a branch for your system, it falls through to `extractPs12Sku` which reads PS1/PS2 disc image headers — for any other format it just returns `null`, and every game shows up unmatched. Add a branch. The pragma is to parse the filename for whatever key LaunchBox uses (Switch = 16-hex title ID in `[…]`, DS = 4-char game code, etc.); deeper solutions read the ROM header but usually need the emulator's encryption keys.
